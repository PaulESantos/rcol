...
> Verification**: `col_check_name()` returns an essential 9-column
> resolution summary showing accepted names, status, and IDs. -
> **Flexible Classification Extraction**: `col_classification()`
...
- **`col_*()`** convenience functions that target the **latest extended
  ...
  is pinned in session (`col_key()`), ensuring long-running jobs are
  never disrupted by mid-session release updates. Use `col_refresh()` to
  re-pin.
